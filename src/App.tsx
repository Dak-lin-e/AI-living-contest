...
const LOAN_FORMULA: FormulaGuide = {
  ...
  compute: ({ principal, rate, years }) => {
    ...
    return [
      ...
      { label: '총 이자', formula: '대출 원금 × 연 금리 × 대출 기간(년)', substituted: `${money(principal)}원 × ${formatRate(rate)} × ${years}년`, result: `${money(totalInterest)}원` },
...
const SAVINGS_FORMULA: FormulaGuide = {
  ...
  compute: ({ principal, rate, years }) => {
    ...
    return [
      { label: '만기 예상 수령액', formula: '원금 × (1 + 연 금리)^유지 기간(년)', substituted: `${money(principal)}원 × (1 + ${formatRate(rate)})^${years}`, result: `${money(finalValue)}원` },
...
export default function App() {
  ...
  const principal = Number(assets) || 0
  const annual = selectedProduct?.annual_rate || 0
  const duration = Number(years) || 1
  const finalValue = principal * Math.pow(1 + annual / 100, duration)
  const gain = finalValue - principal
  const totalReturn = principal ? (gain / principal) * 100 : 0
  const points = useMemo(() => [principal, principal * (1 + annual / 100), principal * Math.pow(1 + annual / 100, 2), finalValue], [principal, annual, finalValue])
  ...
  const generate = async (e?: FormEvent) => {
    ...
    if (duration < 1 || duration > 30) return setError('투자 기간은 1~30년 사이로 입력해 주세요.')
    if (isSavingsProduct) {
    ...
    try {
      const requestedAnnualRate = Number(selectedProduct?.annual_rate ?? (isMortgageLoan ? 3.8 : isRentLoan ? 3.6 : isCreditLoan ? 5.4 : 0))
  ...
  const panels = [{ label: '01 · 나의 출발점', title: '현재 내 자산과 생활을 살펴봐요', text: `${job} · ${incomeLevel} · 월 고정 지출 ${money(Number(monthlyExpenses) || 0)}원인 나의 출발점을 기록했어요.`, type: 'wallet' as const }, { label: '02 · 실제 공시 상품', title: `${selectedProduct?.name || '금융상품'}을 만났어요`, text: `${selectedProduct?.bank_name || '은행'}의 ${selectedProduct?.category || '금융상품'}이에요. 공시 최고금리 연 ${annual.toFixed(2)}%를 기준으로 살펴봐요.`, type: 'shield' as const }, { label: '03 · 시간이 흐르면', title: '작은 차이가 쌓여요', text: `${duration}년 후 예상 수익은 ${money(gain)}원이에요.`, type: 'chart' as const }, { label: '04 · 이야기의 결말', title: '내 자산의 다음 장면', text: `예상 자산은 ${money(finalValue)}원! 실제 적용 조건도 확인해요.`, type: 'star' as const }]
