...
export default function App() {
  ...
  const totalMonths = selectedProduct?.term_months && selectedProduct.term_months > 0 ? selectedProduct.term_months : duration * 12
  const monthSteps = useMemo(() => quarterlyMonths(totalMonths), [totalMonths])
  const points = useMemo(() => monthSteps.map(m => principal * Math.pow(1 + annual / 100, m / 12)), [monthSteps, principal, annual])
  ...
  useEffect(() => {
    if (selectedProduct?.term_months && selectedProduct.term_months > 0) {
      setYears(String(Math.max(1, Math.round(selectedProduct.term_months / 12))))
    }
  }, [selectedProduct?.id])
